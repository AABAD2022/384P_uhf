...
//int LOCK_TRAY_PIN = CONTROLLINO_D0;        // Bloqueo Bandeja
//int REFILL_TRAY_PIN = CONTROLLINO_D1;        // Descarga Bandeja
...
const long interval = 100; // tiempo extra conveyor
...
void setup()
{
  ...
  Controllino_RS485TxEnable();
    Serial.begin(57600);
  //CAMBIO VELOCIDAD LECTOR TARJETA
  delay(500);
  Serial3.begin(115200);
  //Controllino_RS485Init();
  Controllino_RS485TxEnable();
  delay(2000);
  byte trama[] = { 0xBB, 0x00, 0x11, 0X00, 0x02, 0x02, 0x40, 0x55, 0x7E}; //TARMA CAMBIAR VELOCIDAD RFID BB 00 11 00 02 02 40 55 7E
  Serial3.write(trama, sizeof(trama));
  delay(500);
  Serial3.begin(57600);
...
void CONFIG()
{
 ...
 // pinMode(LOCK_TRAY_PIN, OUTPUT);
//  pinMode(REFILL_TRAY_PIN, OUTPUT);
...
{
  Controllino_RS485TxEnable();
  byte trama[] = { 0x03, 0x0F, 0x00, 0x00, 0x00, 0x04, 0x01, 0x00, 0xBF, 0x4F };
  Serial3.write(trama, sizeof(trama));  
int OPEN = READ_PIN(SEN_TRAY);
digitalWrite(LOCK_TRAY_PIN, 0);
 ...
 */
 if (OPEN == 1) {  //clapes en 0
 
 //OPEN = READ_PIN(SEN_TRAY);
 }
 digitalWrite(LOCK_TRAY_PIN, 0);
 /*
 digitalWrite(DTA_WIRE_RED_GND, 0);
...
{
  Controllino_RS485TxEnable();
  byte trama[] = { 0x03, 0x0F, 0x00, 0x00, 0x00, 0x04, 0x01, 0xFF, 0xFF, 0x0F };
  Serial3.write(trama, sizeof(trama));
 // Serial.print("ENVIADO");
  //delay(10);
  int CLOSE = READ_PIN(TRAY_SEN_PIN);
 // Serial.print(CLOSE);
   digitalWrite(LOCK_TRAY_PIN, 0);
  /*
  ...
  */
 if (CLOSE == 1) { //clapes en 0
    CLOSE = READ_PIN(TRAY_SEN_PIN);
    digitalWrite(LOCK_TRAY_PIN, 1);
  }
 // byte trama2[] = { 0x03, 0x0F, 0x00, 0x00, 0x00, 0x04, 0x01, 0x00, 0xBF, 0x4F};
 //  Controllino_RS485TxEnable();

 // Serial3.write(trama, sizeof(trama2)); 
...
void uhf_command(uint8_t com_nub) {
  uint8_t b = 0; // Inicializa una variable para el índice de bytes en el comando
  // Itera a través del arreglo de bytes del comando hasta encontrar el byte de finalización 0x7E
  while (RFID_cmdnub[com_nub][b] != 0x7E) {
    // Envía el byte actual del comando al lector UHF a través de la comunicación serie Serial3
    Serial3.write(RFID_cmdnub[com_nub][b]);
    b++; // Incrementa el índice para pasar al siguiente byte en el comando
  }
  // Después de enviar todos los bytes del comando, envía el byte de finalización 0x7E y una nueva línea y retorno de carro
  Serial3.write(0x7E);
  Serial3.write("\n\r");
